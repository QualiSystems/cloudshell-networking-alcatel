...
class AlcatelGenericSNMPAutoload(object):
    ...
    def _load_snmp_tables(self):
        ...
        for item in self.snmp_handler.get_table(
            "IEEE8023-LAG-MIB", "dot3adAggPortAttachedAggID"
        ).values():
            if item["dot3adAggPortAttachedAggID"] != "0":
                self.port_channel2ports[item["dot3adAggPortAttachedAggID"]].append(
                    item["suffix"]
                )
    ...
    def _get_hw_data(self):
        ...
        for index, attrs in hw_names.items():
            ...
            if hw_name != "":
                ...
                res = {
                    k: {kk: vv.strip("'") for kk, vv in v.items()}
    ...
    def _get_chassis_info(self):
        ...
        for chassis_index, attrs in self.chassis_dict.items():
            ...
            chassis_object = GenericChassis(
                ...
                unique_id="{}.{}.{}".format(
                    self.resource_name, "chassis", chassis_index
                ),
            )

            chassis_object.serial_number = attrs.get("tmnxHwSerialNumber", "")

            chassis_type_id = self.snmp_handler.get_property(
                "TIMETRA-CHASSIS-MIB", "tmnxChassisType", chassis_index
            )
            chassis_object.model = self.snmp_handler.get_property(
                "TIMETRA-CHASSIS-MIB", "tmnxChassisTypeDescription", chassis_type_id
            )
    ...
    def _get_power_ports_info(self):
        ...
        for index, attrs in self.power_supply_dict.items():
            pp_ids = self._get_parent_ids(
                index
            )  # [<Chassis id>, <PowerShelf id>, <Power Port id>]
            ...
            power_port_object = GenericPowerPort(
                ...
                unique_id="{0}.{1}.{2}".format(
                    self.resource_name, "power_port", ".".join(pp_ids)
                ),
            )
            # TODO
    ...
    def _get_modules_info(self):
        ...
        for index, attrs in self.io_modules_dict.items():
            ...
            if len(module_ids) < 2:
                self.logger.error(
                    "Impossible to determine IO Module parent. "
                    "Module HwIndex: {}".format(index)
                )
                continue
                ...
                module_object = GenericModule(
                    ...
                    unique_id="{0}.{1}.{2}".format(
                        self.resource_name, "module", ".".join(module_ids)
                    ),
                )
            else:
                module_id = module_ids[-1]
                module_object = GenericSubModule(
                    ...
                    unique_id="{0}.{1}.{2}".format(
                        self.resource_name, "sub_module", ".".join(module_ids)
                    ),
                )

            module_object.serial_number = attrs.get("tmnxHwSerialNumber", "")
            module_type_id = self.snmp_handler.get_property(
                "TIMETRA-CHASSIS-MIB", "tmnxCardEquippedType", ".".join(module_ids)
            )
            module_object.model = self.snmp_handler.get_property(
                "TIMETRA-CHASSIS-MIB", "tmnxCardTypeDescription", module_type_id
            )

            module_object.version = ""
            self._add_element(
                relative_path="/".join(module_ids), resource=module_object
            )
        ...
        for index, attrs in self.mda_modules.items():
            ...
            module_object.model = self.snmp_handler.get_property(
                "TIMETRA-CHASSIS-MIB", "tmnxMdaTypeDescription", module_type_id
            )
    ...
    def _get_ports_and_portchannels_info(self):
        ...
        port_pattern = re.compile(
            r"^(\d+/)*(?P<sfp_port>[a-zA-Z])?\d+/\d+$"
        )  # 1/1/4 or 1/1/c2/1

        port_hw_table = self.snmp_handler.get_table(
            "TIMETRA-PORT-MIB", "tmnxPortName"
        ).values()
        port_ifindex2name = {
            port_data.get("suffix").split(".")[-1]: "Port {}".format(
                port_data.get("tmnxPortName")
            )
        ...
        for port_data in port_hw_table:
            ...
            if "lag" in port_name:  # Check if it is portchannel
                port_channel = GenericPortChannel(
                    ...
                    unique_id="{0}.{1}.{2}".format(
                        self.resource_name, "port_channel", if_index
                    ),
                )

                port_channel.port_description = self.snmp_handler.get_property(
                    "TIMETRA-PORT-MIB", "tmnxPortDescription", port_hw_index
                ).strip("'")
                ...
                port_object = GenericPort(
                    shell_name=self.shell_name,
                    name="Port {}".format(port_name.replace("/", "-")),
                    unique_id="{0}.{1}.{2}".format(
                        self.resource_name, "port", if_index
                    ),
                )

                port_object.port_description = self.snmp_handler.get_property(
                    "TIMETRA-PORT-MIB", "tmnxPortDescription", port_hw_index
                ).strip("'")

                port_object.l2_protocol_type = self.snmp_handler.get_property(
                    "IF-MIB", "ifType", if_index
                ).strip("'")
                port_object.mac_address = self.snmp_handler.get_property(
                    "IF-MIB", "ifPhysAddress", if_index
                ).strip("'")
                port_object.mtu = self.snmp_handler.get_property(
                    "IF-MIB", "ifMtu", if_index
                )
                port_object.bandwidth = self.snmp_handler.get_property(
                    "IF-MIB", "ifHighSpeed", if_index
                )
    ...
    def _get_ipv4_interface_address(self, port_index):
        ...
        if self.ip_v4_table and len(self.ip_v4_table) > 1:
            for key, value in self.ip_v4_table.iteritems():
                if (
                    "ipAdEntIfIndex" in value
                    and int(value["ipAdEntIfIndex"]) == port_index
                ):
                    return key
    ...
    def _get_duplex(self, port_hw_id):
        ...
        if (
            ...
            or self.snmp_handler.get_property(
                "TIMETRA-PORT-MIB", "tmnxPortEtherOperDuplex", port_hw_id
            ).strip("'")
            == "fullDuplex"
        ):
            return "Full"

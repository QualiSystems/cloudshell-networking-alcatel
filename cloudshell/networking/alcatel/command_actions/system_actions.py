...
class SystemActions(object):
    ...
    def copy_running_config(self, destination, timeout=120):
        CommandTemplateExecutor(
            self._cli_service, configuration.SAVE_RUNNING_CONFIG, timeout=timeout
    ...
    def get_primary_config_path(self):
        ...
        match = re.search(
            r"^\s*primary-config\s+(?P<path>.+)$", bof, flags=re.MULTILINE
        )
        return match.group("path").strip()
    ...
    def reboot(self, timeout=3600, upgrade=False):
        ...
        try:
            CommandTemplateExecutor(
                self._cli_service, configuration.REBOOT
    ...
    def get_bof_source(self):
        ...
        match = re.search(
            r"^\s*BOF Source\s*:\s+(?P<source>.+)\s*$", sys_info, flags=re.MULTILINE
        )

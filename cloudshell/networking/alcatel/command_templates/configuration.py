...
SAVE_BOF = CommandTemplate(
    "bof save", error_map={r"^((?!Completed\.).)*$": Exception("Fail to save BOF")}
)
...
COPY = CommandTemplate(
    "file copy {src} {dst}",
    action_map={
        r"Overwrite destination file:.+\(y/n\)?": lambda s, l: s.send_line("y", l)
    },
...
SAVE_RUNNING_CONFIG = CommandTemplate(
    "admin save {dst}", error_map={r"^((?!Completed\.).)*$": Exception("Fail to save")}
)

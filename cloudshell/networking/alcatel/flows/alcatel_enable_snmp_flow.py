...
from cloudshell.networking.alcatel.command_actions.enable_disable_snmp_actions import (
...
class AlcatelEnableSnmpFlow(EnableSnmpFlow):
    ...
    ALCATEL_SNMP_AUTH_MAP = {
        "No Authentication Protocol": "none",
        "MD5": "md5",
        "SHA": "sha",
    }
    ALCATEL_SNMP_PRIV_MAP = {
        "No Privacy Protocol": "none",
        "DES": "des-key",
    ...
    def _enable_snmp_v2(self, snmp_param):
        ...
        with self._cli_handler.get_cli_service(
            ...
            if not snmp_actions.is_configured(snmp_community):
                access_mode = (
                    "rw" if isinstance(snmp_param, SNMPV2WriteParameters) else "r"
                )
                snmp_actions.enable_snmp(snmp_community, access_mode)
            else:
                self._logger.debug(
                    'SNMP Community "{}" already configured'.format(snmp_community)
                )
                return
    ...
    def _enable_snmp_v3(self, snmp_param):
        ...
        with self._cli_handler.get_cli_service(
            ...
            if snmp_actions.is_exists_user(snmp_param.snmp_user):
                self._logger.debug(
                    'SNMP User "{}" already exists'.format(snmp_param.snmp_user)
                )
                return
    ...
    def _get_security_level(auth_type, priv_type):
        if auth_type == "No Authentication Protocol":
            ...
            auth_type != "No Authentication Protocol"
            and priv_type == "No Privacy Protocol"
        ):
    ...
    def _validate_snmp_v3_params(self, snmp_param, auth_type, priv_type):
        ...
        if not snmp_param.snmp_user:
            ...
            message = message.format(
                "SNMP V3 Authentication Protocol attribute cannot be empty"
            )
        elif not snmp_param.private_key_protocol:
            message = message.format(
                "SNMP V3 Privacy Protocol attribute cannot be empty"
            )

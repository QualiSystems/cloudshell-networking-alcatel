...
class TestSaveConfig(BaseAlcatelTestCase):
    def _setUp(self, attrs=None):
        attributes = {"Backup Location": "folder"}
...
{0}""".format(
                ...
                [
                    ...
                    r"Copying file cf1:\config.cfg ... OK\n1 file copied.\n{}".format(
                        DEFAULT_PROMPT
                    ),
                    1,
                ],
...
{0}""".format(
    ...
    @patch("cloudshell.cli.session.ssh_session.SSHSession._receive_all")
    @patch("cloudshell.cli.session.ssh_session.SSHSession.send_line")
    def test_save_to_default_location(
        ...
        emu = CliEmulator(
            [
                [
                    r"^admin save {}//folder/{}".format(
                        self.runner.file_system, file_pattern
                    ),
                    "Completed.\n{}".format(DEFAULT_PROMPT),
                    1,
                ]

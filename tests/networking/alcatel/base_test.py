        ...
        for action in self.actions:
            ...
            if re.search(command, self.command):
                if calls is None or calls > 0:
                    ...
                    raise ValueError(
                        '"{}" command called more times than needed'.format(
                            self.command
                        )
                    )
    ...
    def check_calls(self):
        for command, response, calls in self.actions:
            if calls is not None and calls != 0:
                raise ValueError(
                    "{} not executed needed times ({} left)".format(command, calls)
                )
...
class BaseAlcatelTestCase(TestCase):
    ...
    def _setUp(self, attrs=None):
        ...
        self.cli_handler = AlcatelCliHandler(
            self._cli, self.resource_config, self.logger, self.api
        )
